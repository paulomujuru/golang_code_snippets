...
func main() {
	...
	fmt.Println("The addition of variables:", v1+v2+v3)
	...
	fmt.Println("This is the memory address of P:", p1)

	p2 := &i                                                                  //create a pointer via type inference
	fmt.Println("'dereference' the pointer and get the original value:", *p2) //use the * in order to 'dereference' the pointer and get the original value

	n := 32
	f := float64(n)
	const pi = 3.14
	fmt.Println("Zuriel to identify the result:", pi*f)
	...
	Zuriel := [8]int{1, 2, 3, 4, 5, 6, 7, 8}
	fmt.Println("Array Zuriel", Zuriel, len(Zuriel))
